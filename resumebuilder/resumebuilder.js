...
ResumeBuilder.prototype.addExperienceModule = function(){
	...
	html.push('Accessible YouTube is an interface to YouTube designed for people who access their computers via either 2 switches for children with disabilities or a mouse. http://gbserver2.cs.unc.edu/playpen/AccessibleYouTube/');
	html.push('</div>');
	html.push('</div>');
	html.push('</div>');
	html.push('</div>');
...
ResumeBuilder.prototype.addEducationModule = function(){
	...
	html.push('<div class="achievements">');
	html.push('Won second place in a programming competition hosted by Passport Parking and now my picture is up on the wall in the computer science building. My team and I placed third out of eight teams in Facebook\'s 2012 North Carolina Hackathon.');
	html.push('</div>');
	html.push('</div>');
	html.push('</div>');
	html.push('</div>');
